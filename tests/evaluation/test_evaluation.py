...
from recsys_platform.evaluation import evaluation
...
def test_evaluate_returns_all_default_metrics(ranking_case) -> None:
    ...
    result = evaluation.evaluate(y_true=y_true, y_pred=y_pred, k=3)
...
def test_evaluate_matches_single_result_from_evaluate_multiple(ranking_case) -> None:
    ...
    single = evaluation.evaluate(y_true=y_true, y_pred=y_pred, k=3)
    multiple = evaluation.evaluate_multiple(y_true=y_true, y_pred=y_pred, k_values=(3,))
...
def test_evaluate_uses_default_k(ranking_case) -> None:
    ...
    default_result = evaluation.evaluate(y_true=y_true, y_pred=y_pred)
    explicit_result = evaluation.evaluate(y_true=y_true, y_pred=y_pred, k=10)
...
def test_evaluate_selected_metrics_only(ranking_case) -> None:
    ...
    result = evaluation.evaluate(y_true=y_true, y_pred=y_pred, k=3, metrics=("precision", "ndcg"))
...
def test_evaluate_multiple_calculates_multiple_cutoffs(ranking_case) -> None:
    ...
    result = evaluation.evaluate_multiple(y_true=y_true, y_pred=y_pred, k_values=(1, 3, 5))
...
def test_evaluate_multiple_sorts_and_deduplicates_k_values(ranking_case) -> None:
    ...
    result = evaluation.evaluate_multiple(y_true=y_true, y_pred=y_pred, k_values=(5, 1, 3, 5, 1))
