...
def test_fit_orders_items_by_popularity(testing_dataset):
    model = PopularityRecommender().fit(testing_dataset)
...
def test_scores_are_normalized(testing_dataset):
    model = PopularityRecommender().fit(testing_dataset)
...
def test_predict_respects_k(testing_dataset):
    model = PopularityRecommender().fit(testing_dataset)

    result = model.predict([RecommendationRequest(user_id=42, k=2)])

    assert [item.item_id for item in result[42]] == [10, 20]


def test_predict_supports_different_k_per_user(testing_dataset):
    model = PopularityRecommender().fit(testing_dataset)
...
def test_predict_with_empty_requests(testing_dataset):
    model = PopularityRecommender().fit(testing_dataset)

    assert model.predict([]) == {}


def test_predict_with_k_larger_than_catalog(testing_dataset):
    model = PopularityRecommender().fit(testing_dataset)

    result = model.predict([RecommendationRequest(user_id=42, k=100)])

    assert len(result[42]) == 3


def test_save_and_load(testing_dataset, tmp_path):
    original = PopularityRecommender().fit(testing_dataset)
    ...
    requests = [RecommendationRequest(user_id=42, k=2)]
